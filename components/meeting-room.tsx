...
export function MeetingRoom() {
  ...
  return (
    <div className="min-h-dvh bg-paper px-3 py-3 text-ink md:px-5">
      <div className="mx-auto flex min-h-[calc(100dvh-1.5rem)] max-w-[1800px] flex-col gap-3 rounded-[34px] border border-ink/10 bg-white/35 p-3 shadow-glow backdrop-blur-xl">
        <header className="panel-surface flex flex-col gap-4 rounded-[28px] px-5 py-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            <div className="mb-2 flex flex-wrap items-center gap-2">
              ...
              {openClawSessionId ? <Badge variant="outline">{copy.meeting.openClawBadge(openClawSessionId.slice(0, 8))}</Badge> : null}
            </div>
            <h1 className="font-display text-3xl font-semibold md:text-4xl">{copy.meeting.title}</h1>
            <p className="mt-2 max-w-4xl text-sm text-mist">{notice}</p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <Button size="sm" variant={sttMode === "browser" ? "default" : "outline"} onClick={() => setSttMode("browser")}>{copy.meeting.browserStt}</Button>
            <Button size="sm" variant={sttMode === "whisper" ? "default" : "outline"} onClick={() => setSttMode("whisper")} disabled={!capabilities?.openaiStt}>{copy.meeting.whisper}</Button>
            <Button size="sm" variant={ttsMode === "browser" ? "default" : "outline"} onClick={() => setTtsMode("browser")}>{copy.meeting.browserTts}</Button>
            <Button size="sm" variant={ttsMode === "elevenlabs" ? "default" : "outline"} onClick={() => setTtsMode("elevenlabs")} disabled={!capabilities?.elevenLabsTts}>{copy.meeting.elevenLabs}</Button>
            <select className="h-9 rounded-full border border-ink/10 bg-white px-3 text-xs" value={autoSpeakMode} onChange={(event) => setAutoSpeakMode(event.target.value as AutoSpeakMode)}><option value="summary">{copy.autoSpeak.summary}</option><option value="all">{copy.autoSpeak.all}</option><option value="off">{copy.autoSpeak.off}</option></select>
          </div>
        </header>

        <div className="grid flex-1 gap-3 xl:grid-cols-[360px_minmax(0,1fr)_430px]">
          <Card className="flex min-h-0 flex-col p-4">
            <div className="mb-4 flex items-center justify-between"><div><div className="text-xs font-semibold uppercase tracking-[0.2em] text-mist">{copy.meeting.workspaceEyebrow}</div><div className="font-display text-2xl">{copy.meeting.workspaceTitle}</div></div><Badge variant="secondary">{labelForBadge(selectedSnapshot?.status || "loading")}</Badge></div>
            <div className="mb-4 grid grid-cols-4 gap-2">{Object.entries(TAB_LABELS).map(([tab, label]) => <Button key={tab} size="sm" variant={activeTab === tab ? "default" : "outline"} onClick={() => setActiveTab(tab as WorkspaceTab)}>{label}</Button>)}</div>
            <ScrollArea className="min-h-0 flex-1 space-y-4 pr-1">
              {activeTab === "btc" ? <div className="space-y-4"><Sparkline snapshot={btcSnapshot} /><QuoteList title={copy.meeting.btckrw} quotes={btcSnapshot?.watchlist ?? []} /></div> : null}
              {activeTab === "kr" ? <div className="space-y-4"><QuoteList title={copy.meeting.kospiKosdaq} quotes={krSnapshot?.indices ?? []} /><QuoteList title={copy.meeting.krWatchlist} quotes={krSnapshot?.watchlist ?? []} /></div> : null}
              {activeTab === "us" ? <div className="space-y-4"><QuoteList title={copy.meeting.usProxies} quotes={usSnapshot?.indices ?? []} /><QuoteList title={copy.meeting.usWatchlist} quotes={usSnapshot?.watchlist ?? []} /></div> : null}
              {activeTab === "trading" ? <div className="space-y-4"><div className="grid grid-cols-2 gap-3"><div className="rounded-[18px] border border-ink/10 bg-white/75 p-3"><div className="text-xs text-mist">{copy.meeting.tradingCash}</div><div className="mt-1 font-semibold">{formatCurrency(portfolio?.cash ?? 0, portfolio?.currency || "KRW")}</div></div><div className="rounded-[18px] border border-ink/10 bg-white/75 p-3"><div className="text-xs text-mist">{copy.meeting.tradingEquity}</div><div className="mt-1 font-semibold">{formatCurrency(portfolio?.equity ?? 0, portfolio?.currency || "KRW")}</div></div></div><div className="space-y-3 rounded-[22px] border border-ink/10 bg-white/75 p-4"><div className="text-xs font-semibold uppercase tracking-[0.2em] text-mist">{copy.meeting.paperTrade}</div><select className="h-11 w-full rounded-full border border-ink/10 bg-white px-4 text-sm" value={tradeForm.symbol} onChange={(event) => setTradeForm((previous) => ({ ...previous, symbol: event.target.value }))}>{tradeSymbols.map((symbol) => <option key={symbol.value} value={symbol.value}>{symbol.label}</option>)}</select><div className="grid grid-cols-2 gap-2"><select className="h-11 rounded-full border border-ink/10 bg-white px-4 text-sm" value={tradeForm.side} onChange={(event) => setTradeForm((previous) => ({ ...previous, side: event.target.value as TradeFormState["side"] }))}><option value="buy">{copy.trading.side.buy}</option><option value="sell">{copy.trading.side.sell}</option></select><select className="h-11 rounded-full border border-ink/10 bg-white px-4 text-sm" value={tradeForm.orderType} onChange={(event) => setTradeForm((previous) => ({ ...previous, orderType: event.target.value as TradeFormState["orderType"] }))}><option value="market">{copy.trading.orderType.market}</option><option value="limit">{copy.trading.orderType.limit}</option></select></div><div className="grid grid-cols-2 gap-2"><Input value={tradeForm.quantity} onChange={(event) => setTradeForm((previous) => ({ ...previous, quantity: event.target.value }))} placeholder={copy.meeting.quantityPlaceholder} inputMode="numeric" /><Input value={tradeForm.limitPrice} onChange={(event) => setTradeForm((previous) => ({ ...previous, limitPrice: event.target.value }))} placeholder={copy.meeting.limitPlaceholder} inputMode="numeric" disabled={tradeForm.orderType === "market"} /></div><Button onClick={() => void handleTradeSubmit()} disabled={isPlacingOrder}>{isPlacingOrder ? copy.meeting.submittingOrder : copy.meeting.submitOrder}</Button></div><QuoteList title={copy.meeting.positions} quotes={(portfolio?.positions ?? []).map((position) => ({ symbol: position.symbol, name: position.name, market: "KRX", price: position.lastPrice, change: position.unrealizedPnl, changePercent: position.unrealizedPnlPercent, currency: portfolio?.currency || "KRW", updatedAt: portfolio?.updatedAt || new Date().toISOString() }))} /><div className="space-y-2"><div className="text-xs font-semibold uppercase tracking-[0.2em] text-mist">{copy.meeting.recentOrders}</div>{orders.slice(0, 5).map((order) => <div key={order.id} className="rounded-[18px] border border-ink/10 bg-white/75 p-3 text-sm"><div className="flex items-center justify-between"><span>{order.symbol} x{formatNumber(order.quantity)}</span><Badge variant="outline">{labelForBadge(order.status)}</Badge></div><div className="mt-1 text-xs text-mist">{labelForOrderSide(order.side)} {labelForOrderType(order.orderType)} {order.fillPrice ? formatCurrency(order.fillPrice) : order.limitPrice ? formatCurrency(order.limitPrice) : ""}</div></div>)}</div></div> : null}
