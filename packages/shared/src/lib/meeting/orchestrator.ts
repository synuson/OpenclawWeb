...
function buildTurnRequest(args: {
  ...
  return locale === "ko"
    ? [
        ...
        finalize
          ? "최종 답변입니다. 메인 채팅에 바로 올라갈 짧은 답변으로 정리하고, 제목과 인사말은 쓰지 마세요."
          : "메인 채팅에 바로 올라갈 짧은 답변으로 정리하고, 제목과 인사말은 쓰지 마세요.",
    ...
    : [
        ...
        finalize
          ? "This is the final answer for the main chat. Keep it concise, natural, and free of headings or greetings."
          : "Write a concise natural reply for the main chat without headings or greetings.",
...
export async function runMeetingRound(request: MeetingRoundRequest): Promise<MeetingRoundResponse> {
  ...
  const firstRawText = await callLLM(provider, {
  ...
  draftTurns.push({
    ...
    visibleText: buildVisibleTurnText(firstSpeakerId, firstRawText, locale),
    timestamp: isoNow()
  });

  if (!mentionedAgentId) {
    const supportHistory = appendHistory(request.history, draftTurns[0]);
    const supportRawText = await callLLM(provider, {
    ...
    draftTurns.push({
      agentId: supportSpeakerId,
      phase: "summary",
      rawText: supportRawText,
      visibleText: buildVisibleTurnText(supportSpeakerId, supportRawText, locale),
      timestamp: isoNow()
    });
  }

  const actions = detectRoundActions(request.message, draftTurns);
  ...
  if (actions[0]) {
    ...
    const finalHistory = draftTurns.reduce(appendHistory, request.history);
    const finalRawText = await callLLM(provider, {
    ...
    draftTurns.push({
      ...
      visibleText: buildVisibleTurnText(researchAgentId, finalRawText, locale),
      timestamp: isoNow()
    });
